...
namespace mca::MI {
    ...
    public:
        TSPCLayout(const int width, const int height, std::unordered_map<std::string, std::any> config)
        {
            diameter = std::any_cast<float>(config["diameter"]);
            this->width = width;
            this->height = height;

            const auto [ltop_x, ltop_y] = std::any_cast<std::pair<float, float>>(config["ltop"]);
            const auto [rtop_x, rtop_y] = std::any_cast<std::pair<float, float>>(config["rtop"]);
            const auto [lbot_x, lbot_y] = std::any_cast<std::pair<float, float>>(config["lbot"]);
            const auto [rbot_x, rbot_y] = std::any_cast<std::pair<float, float>>(config["rbot"]);
