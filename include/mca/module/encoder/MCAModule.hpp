...
#ifndef MCAMODULE_HPP
#define MCAMODULE_HPP

#include "mca/common/pipeline/module.hpp"

namespace mca::module::encoder {
    class MCAModule final : public Module<cv::Mat_C3, cv::Mat_C3, MI::layout_ptr, common::Dict> {
    public:
        cv::Mat_C3 exec(cv::Mat_C3 src, const MI::layout_ptr layout, common::Dict config) override {

            const auto patch_size = std::any_cast<int>(config["patch"]);

            cv::Mat_C3 dst = crop(src, layout, patch_size);

            return std::move(dst);
        }

    private:
        static cv::Mat_C3 crop(cv::Mat_C3& src, const MI::layout_ptr& layout, int patch_size)
        {
